...
func TraceNetwork() {
	...
	eventCallback := func(event *types.Event) {
		// fmt.Printf("%v\n\n", event)
		fmt.Printf("Timestamp: %v, Type: %v, Mount Ns ID: %v, Network Ns ID: %v\n", event.Timestamp, event.Type, event.MountNsID, event.NetNsID)
		fmt.Printf("Packet Type: %s, Protocol: %s, Port: %d, Destination Endpoint: %v\n\n", event.PktType, event.Proto, event.Port, event.DstEndpoint)
	}
